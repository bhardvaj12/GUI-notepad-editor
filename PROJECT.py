            ...
            if dbcursor.fetchone():
                ...
                try:
                    ...
                    with open(os.path.join(os.getcwd(),"notes",filename+".txt"),"w") as f:
                        f.write(f"#created on {time.ctime()} by Ashish mishra\n")
                except:
...
def editor_save(content,filename,current_user):
    """
        This method saves the changes by directly writing the contents from text box to the text file in the system ROM.
    """
